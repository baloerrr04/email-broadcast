...
class AuthService {
    ...
    static async register(email: string, password: string): Promise<User> {
        ...
        if (!newUser) throw new HttpException(500, "User registration failed");
    ...
    static async login(email: string, password: string): Promise<{ token: string, user: User }> {
        ...
        if (!AuthService.JWT_SECRET) throw new HttpException(500, "JWT Secret not configured");
    ...
    static async loginGoogle(profile: Profile): Promise<{ token: string, user: User }> {
        if (!profile.emails || profile.emails.length === 0) throw new HttpException(500, "Google account does not have an email");
        ...
        if (!user) {
            ...
            if (!user) throw new HttpException(500, "User creation failed");
        }
    ...
    static async saveAppPassword(userId: number, appPassword: AppPasswordDTO): Promise<User> {
        ...
        if (!user) throw new HttpException(404, `User with id ${userId} not found`);
        ...
        if (!updatedUser) throw new HttpException(500, "Saving app password failed");
